...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    def IPM_feature(self,
                            value,
                            sampling_locations,
                            value_spatial_shapes,
                            bev_mask):
        bs, num_value, num_heads, embed_dims = value.shape
        _, num_queries, num_heads, _, num_points, _ =\
        ...
        # bs, num_heads*embed_dims, num_queries, num_points
            sampling_value_l_ = F.grid_sample(
                ...
                mode='bilinear',
                padding_mode='zeros',
                align_corners=False).masked_fill(~bev_mask, 0.0)
            sampling_value_list.append(sampling_value_l_)

        # bs, num_heads*embed_dims, num_queries, num_point, lvl ->
        # bs, num_heads*embed_dims, num_queries, num_point*lvl
        output = torch.stack(sampling_value_list, dim=-1).flatten(-2).view(bs, num_heads * embed_dims,
                                                  num_queries,-1)
        output = output.permute(0,2,3,1).reshape(bs, num_queries, -1)
    ...
    def forward(self,
        ...
        num_cam, bs, num_queries, num_points, xy = reference_points_cam.shape
        reference_points = reference_points_cam.reshape(num_cam, num_queries, 1, 1, num_points, xy).expand(-1, -1,
                                                                                                           value.size(
                                                                                                               2),
                                                                                                           spatial_shapes.size(
                                                                                                               0), -1,
        ...
        cam_feat = self.IPM_feature(value,
                                    reference_points,
                                    spatial_shapes,
                                    bev_mask).sum(0, keepdim=True)
